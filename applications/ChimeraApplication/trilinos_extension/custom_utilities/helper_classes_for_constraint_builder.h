...
public:
    ...
    std::string Info() const override
    {
        ...
        buffer <<" #### Slave equation id : "<<Id()<<std::endl;
        int master_index = 0;
        for(auto& master_id : mMasterEquationIdVector)
        {
            buffer<<"\t## Master equation id : "<<master_id<<"\tweight : "<<mMasterWeightsVector[master_index]<<std::endl;
            ++master_index;
        }
        return buffer.str();
    }
...
private:
    ...
    void ModifyLHSForConstraints(MatrixType &rLHSContribution, VectorType& rRHSContribution, EquationIdVectorType &rEquationIds)
    {
        ...
        //                                                                 rLHSContribution(master_index, mLocalIndices.container_master_slaves[master_i_other]);
        ...
        for (auto& slave_index1 : mLocalIndices.slave_index_vector) {
            for (auto& slave_index2 : mLocalIndices.slave_index_vector) {
                rLHSContribution(slave_index1, slave_index2) = 0.0;
                rLHSContribution(slave_index2, slave_index1) = 0.0;
            }
        }
        // For K(s,s)
        for (auto& slave_index1 : mLocalIndices.slave_index_vector) {
                rLHSContribution(slave_index1, slave_index1) = 1.0;
        }
    }
